...
export default function ListAbsence() {
    ...
    const createAbsence = async(e) => {
        ...
        await axios.post("http://localhost:8000/api/tutteur/addAbsence",formData, {
            headers: {
    ...
    const fetchAbsences = async() => {
        try{
            await axios.get(`http://localhost:8000/api/tutteur/showAbsences`, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": 'Bearer ' + accesToken 
                }})
    ...
    const deleteAbsence = async(id) => {
        ...
        if(confirmed) {
            await axios.delete(`http://localhost:8000/api/tutteur/deleteAbsence/${id}`, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": 'Bearer ' + accesToken 
                }})
        ...
        <form action="" onSubmit={createAbsence} className="form" style={{margin:"100px 60px 20px"}}>
            <div className="div">
                    ...
                    filteredAbsence.map((row, key) => (
                        <tr key={key}>
                            ...
                            <td style={{cursor:"pointer"}}>
                                <FontAwesomeIcon onClick={() => deleteAbsence(row.id)} icon={faTrash} />
                            </td>
                            <td>
                                <Link to={`/tutteur/updateAbsence/${row.id}`}><FontAwesomeIcon icon={faEdit} /></Link>
                            </td>
                            </tr>
                        ))
                        )
