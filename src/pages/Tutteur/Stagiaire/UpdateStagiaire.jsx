...
export default function UpdateStagiaire() {
    ...
    const fetchStagiaires = async() => {
        const accesToken = localStorage.getItem('Token_Tutt');
            ...
            await axios.get(`http://localhost:8000/api/tutteur/showStagiaire/${id}`, {
                headers: {
            ...
            .then(({ data })=> {
                console.log(data)
                setCin(data.stagiaire.cin)
                setNom(data.stagiaire.nom)
                setPrenom(data.stagiaire.prenom)
                setEmail(data.stagiaire.email)
                setFill_id(data.stagiaire.fill_id)
                setNumero(data.stagiaire.numero)
                setCef(data.stagiaire.cef)
                setGroup(data.stagiaire.group)
                setAnnee(data.stagiaire.annee)
                setNiveau(data.stagiaire.niveau)
                setSexe(data.stagiaire.sexe)
    ...
    const updateStagiaire = async(e) => {
        ...
        await axios.post(`http://localhost:8000/api/tutteur/updateStagiaire/${id}`,formData, {
            headers: {
                "Accept": "application/json",
                "Authorization": 'Bearer ' + accesToken 
            }})
            .then(({data})=> {
                ...
                console.log(data.message)
                setTimeout(() => navigate("/tutteur/listStagiaire"), 1000);
            }).catch(({response})=>{
...
<div className='content'>
    ...
    <h4 className="h4" style={{margin:"-50px 60px -20px"}}>Modifier Un Stagiaire</h4>
    {alertMessage && <Alert severity={alertSeverity} style={{marginTop:"20px"}}>{alertMessage}</Alert>}
    <form action="" onSubmit={updateStagiaire} className="form">
        <div className="div">
