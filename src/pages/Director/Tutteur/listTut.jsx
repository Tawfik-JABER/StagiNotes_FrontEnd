...
export default function ListTut() {
    ...
    const fetchTutteurs = async() => {
        try{
            await axios.get(`http://localhost:8000/api/director/showTutteurs`, {
                headers: {
    ...
    const deleteFormatteur = async(id) => {
        ...
        const confirmed = confirm(`tu peux suprimer ce Tutteur avec id ${id}`)
        if (confirmed) {
            await axios.delete(`http://localhost:8000/api/director/deleteTutteur/${id}`, {
                headers: {
                    "Accept": "application/json",
                    "Authorization": 'Bearer ' + accesToken 
                }})
...
const logout = () => {
    ...
    return (
                ...
                tutteurs &&(
                    filteredTutteurs.map((row, key) => (
                        <tr key={key}>
                            ...
                            <td>
                                <Link to={`/director/updateTutteur/${row.id}`}><FontAwesomeIcon icon={faEdit} /></Link>
                            </td>
                            </tr>
                        ))
                        )
        ...
        <Link  className="a"  to="/director/createTutteur">Ajouter Un Tutteur</Link>
    </div>
    </>
    )
}
